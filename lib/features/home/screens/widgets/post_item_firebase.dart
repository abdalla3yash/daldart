...
class PostItemFromFirebase extends StatelessWidget {
  FirebasePost firebasePost;
   PostItemFromFirebase({super.key,required this.firebasePost});

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: ()=> ContactHelper.launchURL(firebasePost.permalink),
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(firebasePost.title, style: Theme.of(context).textTheme.labelLarge, maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: AppPadding.p4),
            Text(firebasePost.selfText, maxLines: 3, overflow: TextOverflow.ellipsis, textAlign: TextAlign.start, style: Theme.of(context).textTheme.bodyMedium),
            const SizedBox(height: AppPadding.p4),
          ],
        ),
      ),
